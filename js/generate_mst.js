function generate(rows, cols) {
    ...
    function kruskal() {
        ...
        let mst = Array.from({ length: 2 * rows - 1 }, () => Array(2 * cols - 1).fill(0));
        ...
        while (kol.length) {
            ...
            if (first_anc !== second_anc) {
                ...
                k == 0 ? mst[2 * uRow - 1][(u % cols) * 2] = 1 : mst[2 * uRow][(u % cols) * 2 + 1] = 1;
            }
        ...
        for (let j = 0; j < cols; ++j) {
            ...
            if (j < cols - 1) {
                ...
                if(rows>Math.floor(Math.sqrt(cols)*cols))
                {
                    ...
                    kol.push([weight, i * cols + j, i * cols + j + 1, 1]);
            }
        }
    }
    return kruskal();
...
function startowe(rows, cols, mst) {
    let vis = Array.from({ length: 2 * rows - 1 }, () => Array(2 * cols - 1).fill(false));
    ...
    function dfs(x, y, distance) {
        ...
        for (let [dx, dy] of directions) {
            ...
            if (
                nx >= 0 && ny >= 0 &&
                nx < 2 * rows - 1 && ny < 2 * cols - 1 &&
                !vis[nx][ny] && mst[nx][ny] === 1
            ) {
    ...
    vis = Array.from({ length: 2 * rows - 1 }, () => Array(2 * cols - 1).fill(false));
    let farthest1 = dfs(startX, startY, 0);

    vis = Array.from({ length: 2 * rows - 1 }, () => Array(2 * cols - 1).fill(false));
    let farthest2 = dfs(farthest1.x, farthest1.y, 0);

    return { start: { x: farthest1.x, y: farthest1.y }, end: { x: farthest2.x, y: farthest2.y }, distance: farthest2.distance };
}